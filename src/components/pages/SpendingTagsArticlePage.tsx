...
const ARTICLE = ARTICLES.spendingTags;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "whyCategoriesFail", paragraphs: 2, icon: Layers },
  { key: "meetTags", paragraphs: 2, icon: Tag },
  { key: "tagAsYouGo", paragraphs: 2, icon: Zap },
  { key: "splitTheReceipt", paragraphs: 2, icon: Split },
  { key: "askZeroForTheBill", paragraphs: 2, icon: MessageCircle },
  { key: "oneLabelManyJobs", paragraphs: 2, icon: Briefcase },
] as const;
...
/** "Beyond Categories" (tags) article, shared by the (en) and [locale] routes. */
export async function SpendingTagsArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.spendingTags" });
  ...
  const goalsHref = localizePath(locale, ARTICLES.goalsAndDebt.path);
  const rich = (key: string) =>
    t.rich(key, {
      ...
      link2: (chunks) => (
        <Link href={goalsHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
