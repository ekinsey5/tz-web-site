...
const ARTICLE = ARTICLES.budgetWithPartner;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "oneBudget", paragraphs: 2, icon: Users },
  { key: "roles", paragraphs: 2, icon: KeyRound },
  { key: "inviting", paragraphs: 2, icon: Send },
  { key: "changeAndLeave", paragraphs: 2, icon: DoorOpen },
  { key: "teamPlays", paragraphs: 2, icon: Handshake },
] as const;
...
/** "Budgeting Is a Team Sport" article, shared by the (en) and [locale] routes. */
export async function BudgetWithPartnerArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.budgetWithPartner" });
