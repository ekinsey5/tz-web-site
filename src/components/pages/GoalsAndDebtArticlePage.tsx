...
const ARTICLE = ARTICLES.goalsAndDebt;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "dreamsAreFuel", paragraphs: 2, icon: Heart },
  { key: "honestTradeoff", paragraphs: 2, icon: Scale },
  { key: "dreamVisible", paragraphs: 2, icon: ImageIcon },
  { key: "stayOnPace", paragraphs: 2, icon: Gauge },
  { key: "everySource", paragraphs: 2, icon: Coins },
  { key: "permissionToPause", paragraphs: 2, icon: Pause },
] as const;
...
/** "Saving for a Dream While Killing Your Debt" article, shared by the (en) and [locale] routes. */
export async function GoalsAndDebtArticlePage({ locale }: { locale: Locale }) {
  ...
  const payOffHref = localizePath(locale, ARTICLES.payOffDebtFaster.path);
  ...
  const rich = (key: string) =>
    t.rich(key, {
      ...
      link: (chunks) => (
        <Link href={payOffHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
      link2: (chunks) => (
