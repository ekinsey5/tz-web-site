...
const ARTICLE = ARTICLES.firstBudget;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "whyBudgetsFail", paragraphs: 2, icon: LayoutTemplate },
  { key: "fiveQuestions", paragraphs: 3, icon: MessagesSquare },
  { key: "noJudgment", paragraphs: 2, icon: HeartHandshake },
  { key: "review", paragraphs: 2, icon: ClipboardCheck },
  { key: "firstTenMinutes", paragraphs: 2, icon: Timer },
  { key: "startingPoint", paragraphs: 2, icon: Sprout },
] as const;
...
/** "Your First Budget in 10 Minutes" article, shared by the (en) and [locale] routes. */
export async function FirstBudgetArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.firstBudget" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const relatedHref = localizePath(locale, ARTICLES.makingABudget.path);
