...
const ARTICLE = ARTICLES.windfallInterceptor;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "whyDisappear", paragraphs: 2, icon: Wind },
  { key: "meetInterceptor", paragraphs: 2, icon: Radar },
  { key: "oneTap", paragraphs: 2, icon: Zap },
  { key: "stayInCharge", paragraphs: 2, icon: ShieldCheck },
  { key: "quietUntilNeeded", paragraphs: 2, icon: BellOff },
  { key: "beyondDebt", paragraphs: 2, icon: Sprout },
] as const;
...
/** "What to Do With a Windfall" article, shared by the (en) and [locale] routes. */
export async function WindfallInterceptorArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.windfallInterceptor" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const relatedHref = localizePath(locale, ARTICLES.sinkingFunds.path);
  const related2Href = localizePath(locale, ARTICLES.irregularPaycheck.path);
  const rich = (key: string) =>
    t.rich(key, {
      ...
      link2: (chunks) => (
        <Link href={related2Href} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
