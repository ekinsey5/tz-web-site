...
const ARTICLE = ARTICLES.sinkingFunds;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "onSchedule", paragraphs: 2, icon: CalendarClock },
  { key: "math", paragraphs: 2, icon: Divide },
  { key: "flipSwitch", paragraphs: 2, icon: ToggleRight },
  { key: "honest", paragraphs: 2, icon: AlertCircle },
  { key: "fundOrGoal", paragraphs: 2, icon: Target },
  { key: "spendIt", paragraphs: 2, icon: HandCoins },
] as const;
...
/** "Sinking Funds Explained" article, shared by the (en) and [locale] routes. */
export async function SinkingFundsArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.sinkingFunds" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const relatedHref = localizePath(locale, ARTICLES.envelopeBudgeting.path);
