...
const ARTICLE = ARTICLES.canIAffordThis;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "truePrice", paragraphs: 2, icon: Receipt },
  { key: "askZero", paragraphs: 2, icon: MessageCircle },
  { key: "readingCard", paragraphs: 2, icon: CalendarClock },
  { key: "twoButtons", paragraphs: 2, icon: ArrowLeftRight },
  { key: "whenToRun", paragraphs: 2, icon: Users },
] as const;
...
/** "Can I Afford This?" article, shared by the (en) and [locale] routes. */
export async function CanIAffordThisArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.canIAffordThis" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const snowflakeHref = localizePath(locale, ARTICLES.snowflakePayments.path);
  const strategyHref = localizePath(locale, ARTICLES.payOffDebtFaster.path);
  const rich = (key: string) =>
    t.rich(key, {
      strong: (chunks) => <strong className="font-semibold text-ink-strong">{chunks}</strong>,
      em: (chunks) => <em>{chunks}</em>,
      link: (chunks) => (
        <Link href={snowflakeHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
      link2: (chunks) => (
        <Link href={strategyHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
