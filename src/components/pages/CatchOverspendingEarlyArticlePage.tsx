...
const ARTICLE = ARTICLES.catchOverspendingEarly;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "whySneaks", paragraphs: 2, icon: TrendingUp },
  { key: "gauge", paragraphs: 2, icon: Gauge },
  { key: "alerts", paragraphs: 2, icon: BellRing },
  { key: "courseCorrect", paragraphs: 2, icon: ArrowRightLeft },
  { key: "habit", paragraphs: 2, icon: CalendarCheck },
] as const;
...
/** "Catch Overspending Before It Happens" article, shared by the (en) and [locale] routes. */
export async function CatchOverspendingEarlyArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.catchOverspendingEarly" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const envelopeHref = localizePath(locale, ARTICLES.envelopeBudgeting.path);
  const firstBudgetHref = localizePath(locale, ARTICLES.firstBudget.path);
  const rich = (key: string) =>
    t.rich(key, {
      strong: (chunks) => <strong className="font-semibold text-ink-strong">{chunks}</strong>,
      em: (chunks) => <em>{chunks}</em>,
      link: (chunks) => (
        <Link href={envelopeHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
      link2: (chunks) => (
        <Link href={firstBudgetHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
