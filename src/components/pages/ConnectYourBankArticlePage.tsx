...
const ARTICLE = ARTICLES.connectYourBank;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "bankLinking", paragraphs: 2, icon: Landmark },
  { key: "fileImports", paragraphs: 2, icon: FileUp },
  { key: "smartMatch", paragraphs: 2, icon: Tags },
  { key: "pendingReview", paragraphs: 2, icon: ClipboardCheck },
  { key: "fromInbox", paragraphs: 2, icon: Rocket },
] as const;
...
/** "Get Your Money Into Tether-Zero" article, shared by the (en) and [locale] routes. */
export async function ConnectYourBankArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.connectYourBank" });
  ...
  const pacingHref = localizePath(locale, ARTICLES.catchOverspendingEarly.path);
  const rich = (key: string) =>
    t.rich(key, {
      ...
      link2: (chunks) => (
        <Link href={pacingHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
