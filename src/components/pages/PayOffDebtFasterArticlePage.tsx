...
const ARTICLE = ARTICLES.payOffDebtFaster;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "oneEngine", paragraphs: 2, icon: Flame },
  { key: "avalanche", paragraphs: 2, icon: TrendingDown },
  { key: "snowball", paragraphs: 2, icon: Snowflake },
  { key: "simulator", paragraphs: 2, icon: Columns2 },
  { key: "commit", paragraphs: 2, icon: Handshake },
] as const;
...
/** "Snowball vs. Avalanche" article, shared by the (en) and [locale] routes. */
export async function PayOffDebtFasterArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.payOffDebtFaster" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const simulatorHref = `${localizePath(locale, "/")}#debt-crusher`;
  ...
  const rich = (key: string) =>
    t.rich(key, {
      ...
      link: (chunks) => (
        <Link href={simulatorHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
      link2: (chunks) => (
