...
const ARTICLE = ARTICLES.aiFinancialCoach;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "askAnywhere", paragraphs: 2, icon: MessageCircle },
  { key: "realAnswers", paragraphs: 2, icon: Calculator },
  { key: "coachsCorner", paragraphs: 2, icon: Sparkles },
  { key: "inControl", paragraphs: 2, icon: ShieldCheck },
  { key: "gettingStarted", paragraphs: 2, icon: Lightbulb },
] as const;
...
/** "Meet Zero, Your AI Money Coach" article, shared by the (en) and [locale] routes. */
export async function AiFinancialCoachArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.aiFinancialCoach" });
  const tLearn = await getTranslations({ locale, namespace: "Learn" });

  const affordHref = localizePath(locale, ARTICLES.canIAffordThis.path);
  const windfallHref = localizePath(locale, ARTICLES.windfallInterceptor.path);
  const rich = (key: string) =>
    t.rich(key, {
      strong: (chunks) => <strong className="font-semibold text-ink-strong">{chunks}</strong>,
      em: (chunks) => <em>{chunks}</em>,
      link: (chunks) => (
        <Link href={affordHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
      link2: (chunks) => (
        <Link href={windfallHref} className="font-medium text-brand hover:underline">
          {chunks}
        </Link>
      ),
    });
