...
const ARTICLE = ARTICLES.irregularPaycheck;

const P_CLASS = "mt-5 leading-relaxed text-body";

/** Every section is uniform: one h2 plus prose paragraphs. */
const SECTIONS = [
  { key: "monthlyLie", paragraphs: 2, icon: CalendarX2 },
  { key: "realMoney", paragraphs: 2, icon: Banknote },
  { key: "cadenceMath", paragraphs: 2, icon: Repeat },
  { key: "holdingBuffer", paragraphs: 2, icon: Inbox },
  { key: "releaseFunds", paragraphs: 2, icon: Unlock },
  { key: "rhythm", paragraphs: 2, icon: Waves },
] as const;
...
/** "Budgeting on an Irregular Paycheck" article, shared by the (en) and [locale] routes. */
export async function IrregularPaycheckArticlePage({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: "Articles.irregularPaycheck" });
